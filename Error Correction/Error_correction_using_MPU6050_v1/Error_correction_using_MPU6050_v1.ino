...
// It is then converted from radians to degrees
    ...
    //if angle is more than 5 degrees below or above setpoint, OUTPUT will be set to min or max respectively
    myPID.setBangBang(5);
    //set PID update interval to 3000ms
    myPID.setTimeStep(3000);
...
void loop()
{
    ...
    if (outputVal > 0)
    {
        fr_increment += outputVal;
        br_increment += outputVal;
        Serial.print("Increased PWM on right side motors to ");
        Serial.println(fr_increment);
    }
    else
    {
        fl_increment += outputVal;
        bl_increment += outputVal;
        Serial.print("Increased PWM on left side motors to ");
        Serial.println(fl_increment);
    }

    // Rotate all motors forward after the increment based on gyroscope readings
    analogWrite(FL_FWD, fl_increment);
    analogWrite(FR_FWD, fr_increment);
    analogWrite(BL_FWD, bl_increment);
    analogWrite(BR_FWD, br_increment);
    ...
    delay(2); // buffer time to ease readability
}